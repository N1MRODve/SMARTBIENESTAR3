...
export const getResultadosEncuestaById = async (encuestaId) => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      ...
      if (encuesta) {
        // Usar las preguntas existentes si ya están definidas, o buscar en plantilla
        let preguntasBase = encuesta.preguntas || [];

        if (preguntasBase.length === 0 && encuesta.plantilla_id) {
          const plantilla = plantillasDb.value.find(p => p.id === encuesta.plantilla_id);
          if (plantilla) {
            preguntasBase = plantilla.preguntas;
          }
        }

        // Mapear preguntas con resultados formateados para gráficos
        const preguntasConResultados = preguntasBase.map((pregunta, index) => {
          const preguntaId = pregunta.id || `p${index + 1}`;
          let resultados = null;
        ...
        const encuestaConResultados = {
          ...encuesta,
          preguntas: preguntasConResultados,
          // Asegurar que tenemos comentarios_destacados
          comentarios: encuesta.comentarios_destacados || [],
          // Asegurar que tenemos acciones_recomendadas
          recomendaciones: encuesta.acciones_recomendadas || []
        };

        resolve(encuestaConResultados);
      } else {
