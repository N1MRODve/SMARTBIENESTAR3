/*
  # Corrección de fn_submit_encuesta para usar esquema correcto

  ## Cambios realizados

  1. **Corrección de columnas**
    - Usa "departamento" (text) en lugar de "departamento_id" (uuid)
    - Obtiene el nombre del departamento mediante JOIN
    - Usa las columnas correctas: submitted_at, puntos_otorgados, bonus_aplicado

  2. **Compatibilidad**
    - Mantiene toda la lógica de privacidad
    - Funciona con el esquema real de la base de datos
*/
...
BEGIN
  ...
  -- Obtener empleado con nombre de departamento
  SELECT 
    e.id, 
    e.empresa_id, 
    e.puntos,
    d.nombre as departamento
  INTO v_empleado_id, v_empleado_empresa_id, v_empleado_puntos, v_empleado_departamento
  FROM empleados e
  ...
  LOOP
    INSERT INTO respuestas_encuesta (
      ...
      CASE
        WHEN v_privacidad_nivel = 'anonimato_parcial' THEN v_empleado_departamento
        WHEN v_privacidad_nivel = 'identificado' THEN v_empleado_departamento
        ELSE NULL
      END,
      p_hash_ip,
